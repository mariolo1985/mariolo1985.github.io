...
namespace TrainingRequestWFProj.TrainingMaterialsWF
{
    public sealed partial class TrainingMaterialsWF : SequentialWorkflowActivity
    {
            ...
            if ((!string.IsNullOrEmpty(ItemRequestType)) && (!string.IsNullOrEmpty(ItemPlannedTraining)) && (!string.IsNullOrEmpty(ItemCurrentStage)) && (!string.IsNullOrEmpty(ItemDept)))
            {
                ...
                try
                {
                    if ((NewWFStage != "999") && (TaskContentType != null))
                    {
                        ...
                        TrainingTaskProperties.SendEmailNotification = true;// Send Email to Task Owner
                    }
                }
                catch (Exception Err)
                {
                    ...
                    using (SPWeb TrainingWeb = TrainingSite.OpenWeb())
                    {
                        ...
                        try
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    EmailHelper.SendNotification(Assignee.Email, subject, body);// Sends task email
                                }
                            }
                            catch
                            {
                                ...
                                if (!string.IsNullOrEmpty(address))
                                {
                                    ...
                                    EmailHelper.SendNotification(address, subject, body);// Sends task email
                                }
                            }
        ...
        private void OnTaskChanged(object sender, ExternalDataEventArgs e)
        {
            ...
            LogHistoryUserGuid = SPContext.Current.Web.CurrentUser.ID;
                ...
                SPSecurity.RunWithElevatedPrivileges(() =>
                {
                    ...
                    using (SPWeb TrainingWeb = TrainingSite.OpenWeb())
                    {
                        ...
                        if (TaskItemContentType.Name == Constants.AssignTaskContentType)
                        {
                            ...
                            SPField AssignTaskOwnerField = TaskItemContentType.Fields["AssignTaskOwner"];
                            SPField RoleTypeField = TaskItemContentType.Fields["RoleType"];
                            ...
                            if (!string.IsNullOrEmpty(TaskItem[AssignTaskOwnerField.InternalName.ToString()].ToString()))
                            {
                                ...
                                if (RoleType == Constants.TrainerRoleString)
                                {
                                    ...
                                    GetItemMetadata.SetFieldMetadata(TrainingWeb, RequestItem, TrainerField, AssignTaskOwnerCollection);
                                }
                                else
                                {
                        ...
                        else
                        {
                            ...
                            if (!string.IsNullOrEmpty(WFTaskAfterProperties.ExtendedProperties[StatusField.Id].ToString()))
                            {
                                ...
                                if (Status == "Reviewed")
                                {
                                    ...
                                    string ModifiedByName = TaskItem["Modified By"].ToString();
                                    ModifiedByName = ModifiedByName.Substring(ModifiedByName.IndexOf(@"#") + 1);
                                    LogHistory = string.Format("COMPLETED: The task has been reviewed by {0}", ModifiedByName);
